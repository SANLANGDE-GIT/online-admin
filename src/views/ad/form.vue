...
export default {
  ...
  methods: {
    ...
    initAdTypeList() {
      adTypeApi.getListAll().then(response => {
    ...
    saveData() {
      // debugger
      adApi.saveAd(this.ad).then(response => {
    ...
    updateData() {
      // teacher数据的获取
      adApi.updateAd(this.ad).then(response => {
        this.$message.success(response.message)
        this.$router.push({ path: '/ad/list' })
      })
    },
    ...
    // 根据id查询记录
    fetchDataById(id) {
      adApi.getAdInfo(id).then(response => {
    ...
    handleAvatarSuccess(res, file) {
      ...
      if (res.success) {
        // console.log(res)
        this.ad.imageUrl = res.data.path
        // 强制重新渲染
        this.$forceUpdate()
      } else {
